...
def get_optimized_signals(df):
    ...
    # 賣出：(MACD死叉 且 價格弱勢) 或 (RSI過熱) 或 (跌破月線)
    sell = ( (m_dead & (df['Close'] < df['MA5'])) | (df['RSI'] > 75) | ((df['Close'].shift(1) > df['MA20']) & (df['Close'] < df['MA20'])) )
    return buy, sell

# --- 歷史回測與 MPT 引擎 ---
@st.cache_data(ttl=3600)
def get_hist_data_engine(symbols, period="1y"):
...
def perform_mpt_simulation(portfolio_df):
    ...
    try:
        data = get_hist_data_engine(symbols, period="3y").dropna()
        ...
        for i in range(num_p):
            ...
            results[0,i], results[1,i], results[2,i] = p_ret, p_std, (p_ret - 0.02) / p_std
        idx = np.argmax(results[2])
        comp = pd.DataFrame({"股票代號": symbols, "目前權重 (%)": (portfolio_df["現值_TWD"] / portfolio_df["現值_TWD"].sum() * 100).values, "建議權重 (%)": w_record[idx] * 100})
...
def display_market_table(df, title, currency, current_user):
    ...
    st.markdown("---")
    f_cols = st.columns(COLS_RATIO); f_f, f_c = ("{:,.0f}" if currency == "TWD" else "{:,.2f}"), ("red" if s_prof > 0 else "green")
...
with st.sidebar:
    ...
    if current_user != "All":
        with st.form("add_form", clear_on_submit=True):
            s_in = st.text_input("股票代號 (如 2330.TW)").upper().strip()
            q_in, c_in = st.number_input("股數", min_value=0.0), st.number_input("持有成本", min_value=0.0)
            if st.form_submit_button("執行新增持股"):
                if s_in:
...
tab1, tab2, tab3 = st.tabs(["📊 庫存配置與回測", "🧠 技術診斷", "⚖️ 組合優化 (MPT)"])
...
if not df_raw.empty:
    ...
    with tab1:
        ...
        # --- 圓餅圖 ---
        st.divider(); col_p1, col_p2 = st.columns([1, 2])
        with col_p1:
            view_mode = st.selectbox("配置分析範圍：", ["全部", "台股", "美股"], key="p_select")
        with col_p2:
            p_df = portfolio[portfolio["幣別"] == ("TWD" if view_mode == "台股" else "USD")] if view_mode != "全部" else portfolio
            if not p_df.empty: st.plotly_chart(px.pie(p_df, values="現值_TWD", names="股票代號", title=f"{view_mode} 配置佔比", hole=0.4), use_container_width=True)

        # --- 歷史回測 ---
        st.divider(); st.subheader("📈 組合淨值歷史回測 (過去一年)")
        h_data = get_hist_data_engine(portfolio["股票代號"].tolist())
        if not h_data.empty:
            eq_curve = pd.Series(0.0, index=h_data.index)
            fx_h = h_data["USDTWD=X"].ffill()
            for _, row in portfolio.iterrows():
                p_h = h_data[row["股票代號"]].ffill()
                eq_curve += p_h * row["股數"] * (fx_h if row["幣別"]=="USD" else 1.0)
            fig_h = go.Figure(data=go.Scatter(x=eq_curve.index, y=eq_curve, name="組合淨值", line=dict(color='#00D1FF', width=3)))
            fig_h.update_layout(height=400, template="plotly_dark", margin=dict(l=10, r=10, t=30, b=10))
    ...
    with tab2:
        ...
        period = st.select_slider("資料時間範圍", options=["1mo", "3mo", "6mo", "1y"], value="1y")
        df_t = yf.Ticker(target).history(period=period)
        if not df_t.empty:
            df_t = calculate_indicators(df_t)
            df_t['Buy'], df_t['Sell'] = get_optimized_signals(df_t)
    ...
    with tab3:
        ...
        if st.session_state.mpt_results:
            ...
            with ca:
                f_m = px.scatter(r['sim_df'], x='Volatility', y='Return', color='Sharpe', title="效率前緣雲圖")
                f_m.add_trace(go.Scatter(x=[r['max_s'][1]], y=[r['max_s'][0]], mode='markers', marker=dict(color='red', size=15, symbol='star')))
